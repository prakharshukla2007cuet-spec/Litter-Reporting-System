...
if location:
    ...
    try:
        ...
        if loc:
            ...
            st.info(f"📌 Coordinates from address: {latitude:.6f}, {longitude:.6f}  (± {accuracy:.0f} m accuracy)")
    except Exception:
...
# ------------------------------------------------------------------
predicted_type, classification_mode = None, None
...
if image is not None:
    ...
    predicted_type, classification_mode = classify_garbage(temp_path)

    st.subheader("CNN Garbage Classification")
    mode_note = "using trained CNN model" if classification_mode == "CNN" else \
                "using fallback rule-based classifier"
    st.info(f"Predicted type: **{predicted_type.upper()}**  ({mode_note})")
